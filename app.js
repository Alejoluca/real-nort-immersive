...
const metrics = {
  ...
  summary() {
    const c = {};
    this.events.forEach(e => { c[e.name] = (c[e.name] || 0) + 1; });
    return c;
  }
};
...
function setupAfterBuild() {
  ...
  slides().forEach((_, i) => {
    ...
    d.addEventListener("click", () => slides()[i].scrollIntoView({ behavior: "smooth", block: "start" }));
  ...
  const bgObs = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      ...
      if (url) lazyBg(el, url, safeImg(featured[0], 0));
    });
  ...
  gallery.addEventListener("wheel", (e) => {
    ...
    const next = Math.max(0, Math.min(list.length - 1, currentIndex + dir));
  ...
  document.addEventListener("click", (e) => {
    if (e.target && (e.target.id === "btnAllIntro" || e.target.id === "btnAllEnd")) {
      metrics.track("open_grid", { source: e.target.id });
      openAll();
    }
    const btn = e.target.closest("[data-open]");
    if (btn) {
      const id = btn.getAttribute("data-open");
      metrics.track("open_detail", { id, source: "slide" });
      const p = findProp(id);
      if (p) openDetail(p);
    }
    const wa = e.target.closest("[data-wa]");
    if (wa) metrics.track("wa_click", { id: wa.getAttribute("data-wa"), source: "slide" });
  });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (document.getElementById("allOverlay").classList.contains("open")) {
    ...
    if (e.key === "ArrowDown" || e.key === "PageDown") {
      e.preventDefault();
      const n = Math.min(currentIndex + 1, list.length - 1);
      if (n !== currentIndex) list[n].scrollIntoView({ behavior: "smooth", block: "start" });
    }
    if (e.key === "ArrowUp" || e.key === "PageUp") {
      e.preventDefault();
      const n = Math.max(currentIndex - 1, 0);
      if (n !== currentIndex) list[n].scrollIntoView({ behavior: "smooth", block: "start" });
    }
  });
  document.addEventListener("submit", (e) => {
    ...
    const interest = f.querySelector('[name="interest"]');
    const message = f.querySelector('[name="message"]');
    f.querySelectorAll(".field-error").forEach(el => el.remove());
    [name, phone, email].forEach(el => el && el.classList.remove("invalid"));
    let ok = true;
    ...
    if (!name.value.trim()) show(name, "Ingresa tu nombre");
    if ((phone.value || "").replace(/\D/g, "").length < 10) show(phone, "Mín. 10 dígitos");
...
function findProp(id) {
  return (allProperties || []).find(p => p.id === id) || (featured || []).find(p => p.id === id);
}
...
function openDetail(p) {
  ...
  currentDetail = p;
  detailIdx = 0;
  detailImgs = (p.images && p.images.length) ? p.images : [safeImg(p, 0)];
  const slidesEl = document.getElementById("detailSlides");
  const nav = document.getElementById("detailNav");
  const info = document.getElementById("detailInfo");
  slidesEl.innerHTML = detailImgs.map((src, i) => {
    if (i === 0) return `<div class="detail-slide loaded" style="background-image:url('${src}')" data-src="${src}"></div>`;
    return `<div class="detail-slide" data-src="${src}"></div>`;
  }).join("");
  ...
  metrics.track("detail_view", { id: p.id });
}
...
function loadDetailImg(i) {
  ...
  if (!src) return;
  lazyBg(el, src, safeImg(currentDetail, 0));
}
...
function goDetail(i) {
  ...
  detailIdx = ((i % n) + n) % n;
  document.getElementById("detailSlides").style.transform = "translateX(-" + (detailIdx * 100) + "%)";
  document.getElementById("detailNav").querySelectorAll(".detail-dot").forEach((d, idx) => d.classList.toggle("active", idx === detailIdx));
  loadDetailImg(detailIdx);
  loadDetailImg(detailIdx + 1);
  loadDetailImg(detailIdx - 1);
  metrics.track("detail_image", { id: currentDetail.id, index: detailIdx });
}
...
function renderGrid() {
  const filters = [
    { key: "all", label: "Todas" },
    { key: "studio", label: "Estudios / Loft" },
  ...
  const filtered = currentFilter === "all"
    ? (allProperties || [])
    : (allProperties || []).filter(p => p.bedsKey === currentFilter);
  metrics.track("filter", { key: currentFilter, count: filtered.length });
  document.getElementById("filterBar").innerHTML =
    filters.map(f => `<button type="button" class="filter-btn${currentFilter===f.key?" active":""}" data-f="${f.key}">${f.label}</button>`).join("")
    + `<span class="filter-count">${filtered.length} propiedades</span>`;
  document.getElementById("filterBar").querySelectorAll(".filter-btn").forEach(b => {
    b.addEventListener("click", () => { currentFilter = b.dataset.f; renderGrid(); });
  });
  const grid = document.getElementById("propGrid");
  grid.innerHTML = filtered.map(p => {
    const img = safeImg(p, 0);
    return `<div class="card" data-id="${p.id}"><div class="card-img" data-bg="${img}"></div><div class="card-body"><div class="card-tag">${p.loc || "Tulum"}</div><div class="card-name">${p.name}</div><div class="card-meta">${p.beds || ""}</div><div class="card-price">${p.price || "Consultar"}</div></div></div>`;
  }).join("");
  ...
  gridImgObs = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      ...
      const el = e.target;
      const url = el.getAttribute("data-bg");
      if (url) lazyBg(el, url, safeImg(null, 0));
      gridImgObs.unobserve(el);
    });
  }, { root: grid, rootMargin: "200px 0px", threshold: 0.01 });
  grid.querySelectorAll(".card-img[data-bg]").forEach(el => gridImgObs.observe(el));
  grid.querySelectorAll(".card").forEach(card => {
    card.addEventListener("click", () => {
      const p = findProp(card.dataset.id);
      if (p) {
        metrics.track("open_detail", { id: p.id, source: "grid" });
        closeAll();
        openDetail(p);
      }
    });
...
if (typeof featured !== "undefined" && featured.length) buildGallery();
else setTimeout(() => buildGallery(), 50);
})();
